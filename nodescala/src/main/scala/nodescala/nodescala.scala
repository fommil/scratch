...
trait NodeScala {
  ...
  /** Uses the response object to respond to the write the response back.
    ...
    */
  private def respond(exchange: Exchange, token: CancellationToken, response: Response) {
  ...
  def start(relativePath: String)(handler: Request ⇒ Response): Subscription = {
    ...
    val loop = Future.run() { token ⇒
      def serve: Unit = if (token.nonCancelled) {
...
object NodeScala {
  ...
  trait Listener {
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      ...
      createContext { e =>
        promise.completeWith(Future(e.request, e))
        removeContext()
      }
      promise.future
    }
