...
//Version 1.5
//
// Updates:     - smart edge normalisation (not always too smart though) it takes the average
//                of the outputed filtered values, and corrects for a partially used filter...
//                this works best with filters of total area of unity, least well with filters
//                with negative and positive values, were results are sporatic, but no divide
//                by zero bugs at least if the total area of the filter is zero.
//
// BUGS:	- doesnt support even numbered filters too well.. output x-offset by 1/2 row
//              - filters with negative values do not work well with the edge effects
//
...
for(conv_j=0,conv_sumfilter=0;conv_j<conv_filtersize;conv_j++)
        {
        conv_sumfilter=conv_sumfilter+sqrt(pow(conv_filter[conv_j],2));
        }
...
for(conv_j=0;conv_j<(conv_filtersize/2);conv_j++,conv_tempsumfilter=0,conv_temp=0)
	...
	for(conv_k=((conv_filtersize/2)-conv_j);conv_k<(conv_filtersize);conv_k++)
		{
		conv_output[conv_j]=conv_output[conv_j]+((conv_input[conv_j+conv_k-(conv_filtersize/2)])*(conv_filter[conv_k]));
                conv_tempsumfilter=conv_tempsumfilter+sqrt(pow(conv_filter[conv_k],2));
                }
        if((conv_tempsumfilter!=0)&&(conv_sumfilter!=0))
                {
...
if(conv_edges==1)
{
for(conv_j=1,conv_tempsumfilter=0;conv_outpos<conv_size;conv_outpos++,conv_j++,conv_tempsumfilter=0,conv_temp=0)
	{
	for(conv_k=0;conv_k<(conv_filtersize-conv_j);conv_k++)
		{
		conv_output[conv_outpos]=conv_output[conv_outpos]+((conv_input[conv_outpos+conv_k-conv_temp2])*(conv_filter[conv_k]));
                conv_tempsumfilter=conv_tempsumfilter+sqrt(pow(conv_filter[conv_k],2));
                }
        if((conv_tempsumfilter!=0)&&(conv_sumfilter!=0))
                {
