...
This version handles:
	addition of 2 numbers (integer or decimal)
	addition of 2 negative vales, rejection of + and  - addition
	redundant zero and + symbol removal at start of number string
	overflow detection

Does not:
	remove redundant zeros at end of decimal point
	handle scientific notation
	
Short version:
	Library of math functions (this is the add part) which perform the same functions
	in the standard package only numbers are stored in string arrays, and the maximum
	number size is then controlled at runtime.
...
Functions still to create:
	General printer of the array format
	subtraction
	multiplication
	division
	modulus
...
static char digit,carry,larger,logizero,logidecimal,decnumbers,largerafterdecimal,ve1=1,ve2=1;
...
ea1++;ea2++;
	if((a1d)>(a2d)){larger=1;eaa=a1d+1;aad=a1d+1;addarray[aad]=DECMARK;}
	else if((a1d)<(a2d)){larger=2;eaa=a2d+1;aad=a2d+1;addarray[aad]=DECMARK;}
	else{larger=0;eaa=a1d+1;aad=a1d+1;addarray[aad]=DECMARK;}
...
if((ve1==1)&&(ve2==1))addarray[0]=0;
else if((ve1==-1)&&(ve2==-1))addarray[0]=NEGATION;
else {printf("\nERROR: a negative and a positive value sent to SAMADD function");}
addarray[1]=carry;
//-----------------------------------------------------------------------------
//Print out
...
printf("\n=\n");*/
if((eaa>=MAXDIGITS+2)&&(addarray[1]==1)){printf("WARNING WARNING WARNING WARNING WARNING WARNING WARNING\nAnswer out of limits!!!!\n");}
}
...
printf("%c\n",outaddarray[0]);
if(outaddarray[0]==NEGATION){printf("-");}
if(outaddarray[1]==1){printf("1");}
for(i=2;outaddarray[i]!=ENDMARK;i++)
	{
