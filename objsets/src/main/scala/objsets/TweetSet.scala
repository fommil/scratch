...
abstract class TweetSet {
  ...
  //    @tailrec
  //    def rec(list : TweetSet, set: TweetSet): TweetSet = {
  //      if (list.isEmpty) set
  //      else rec(list.tail, set.incl(list.head))
  //    }
  //    rec(that, this)
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = {
    if (p(head))
      new NonEmpty(head, accu, tail.filter0(p, accu))
    else
      tail.filter0(p, accu)
  }
...
//    TweetReader.allTweets.filter(t => !google.filter(t.text.contains(_)).isEmpty)
